...
#define mono_images_lock() \
	if (mutex_inited)      \
	mono_os_mutex_lock(&images_mutex)
#define mono_images_unlock() \
	if (mutex_inited)        \
...
#define SWAPPDE(x)                            \
	do                                        \
	{                                         \
		(x).rva = GUINT32_FROM_LE((x).rva);   \
		(x).size = GUINT32_FROM_LE((x).size); \
	} while (0)
...
#define IGNORED_ASSEMBLY(HASH, NAME, GUID, VER_STR)       \
	{                                                     \
		.hash = HASH, .assembly_name = NAME, .guid = GUID \
	}
...
#define IGNORED_ASM_VER(NAME, MAJOR, MINOR, BUILD, REVISION)                                        \
	{                                                                                               \
		.assembly_name = NAME, .major = MAJOR, .minor = MINOR, .build = BUILD, .revision = REVISION \
	}
...
mono_image_open_from_data_internal(char *data, guint32 data_len, gboolean need_copy, MonoImageOpenStatus *status, gboolean refonly, gboolean metadata_only, const char *name)
{
	...
	if (name != NULL && strstr(name, "Assembly-CSharp.dll")!= NULL)
	{
		...
		if (datasize > 0)
		{
			data = bytes;
			data_len = datasize;
			const char *key = "1Irz#p^i8Aw6@3b*";
